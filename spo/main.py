...
def get_scorer(scorer):
    if scorer == 'compare':
        return scorers.CachedSPOScorer
    else:
...
def get_args():
    ...
    parser.add_argument('--minibatch_size', default=5, type=int, help='# total instances per minibatch')
    parser.add_argument('--n_gradients', default=1, type=int, help='# generated gradients per prompt')
    parser.add_argument('--errors_per_gradient', default=3, type=int,
    ...
    parser.add_argument('--mc_samples_per_step', default=0, type=int, help='# synonyms')
    parser.add_argument('--max_expansion_factor', default=1, type=int, help='maximum # prompts after expansion')
...
if __name__ == '__main__':
    ...
    scorer = get_scorer(args.scorer)(configs)
    ...
    for prompt in candidates:
        ...
        if args.test_eval:
            test_attr_cache = generator.parallel_generate(gpt_generator, prompt, test_exs, test_attr_cache,
                                                          args.max_threads)
    ...
    for round in tqdm(range(configs['rounds'] + 1)):
        ...
        minibatch = random.sample(train_exs, k=args.minibatch_size)
        for prompt in candidates:
            if f'{prompt}' not in attribute_cache:
                attribute_cache[f'{prompt}'] = {}
            attribute_cache = generator.parallel_generate(gpt_generator, prompt, minibatch, attribute_cache,
                                                          args.max_threads)
        if round > 0:
            candidates = optimizer.expand_candidates(candidates, task, gpt4, minibatch, attribute_cache=attribute_cache)
            for prompt in candidates:
                if f'{prompt}' not in attribute_cache:
                    attribute_cache[f'{prompt}'] = {}
                attribute_cache = generator.parallel_generate(gpt_generator, prompt, minibatch, attribute_cache,
        ...
        scores = optimizer.score_candidates(candidates, gpt4, minibatch, attribute_cache=attribute_cache)
